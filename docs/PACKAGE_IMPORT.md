...
- **Package header**: first line → title (unless it is itself an editor line, in which case the package keeps no title rather than being named after its editor); a short second line → subtitle appended to the title; an editor line — «Редактор …», «Автор тем - …», «Редактор та автор тем — …», «Редактор та автор усіх запитань: …» (with optional prefix like «Коло 1.») → package editors (`SharedEditors`); everything else → preamble. The scope qualifier after the role («тем», «запитань», «усіх запитань») is enumerated, so an acknowledgment line like «Редактор вдячний за тестування: …» is not mistaken for editors
...
- **«Теми:» list**: entries (optionally numbered «1.») become *anchors* — they anchor detection of bare-title theme headers later in the document and stay in the preamble. The real content is detected by a duplicate title or a title line standing right before the first «10.» question. Titles are matched with surrounding quotes and terminal punctuation stripped, so a header written «"БАНДУРИСТ".» still meets its list entry «Бандурист»
- **Theme list without a «Теми:» header**: many packages simply print the list. An unbroken «1., 2., 3., …» run of short title-like entries in the package header is read as the list, giving the same anchors. Entries must be ≤70 chars, which is what keeps numbered *host instructions* («1. Ознайомитися з темами…») from being taken for themes. Contiguity separates the list from the real headers, which carry the same numbers but are broken up by their questions
- **Position-numbered headers**: a header stating its own position («5. Я І МОЇ КОЗИ» opening the fifth theme) is accepted on the number alone, so it works with no theme list and with a preamble standing between the title and «10.». Narrowly guarded — the previous theme must be complete, the line must not continue a numbered run, and the next question in the document must be a «10.» — because a numbered line is just as often an enumeration inside a comment
- **Headers vs. preambles**: the paragraph after a theme header is that theme's preamble, never a second header. An explicit «Тема …» line is therefore ignored while the current theme is still empty, so preamble prose like «Тема - набірна матриця. Всі відповіді…» does not replace the title
...
- **Author cascade**: a question without an explicit «Автор:» inherits the theme authors (from the header parens), or the package-level editor when the theme has none — most packages have one global author. An explicit «Автор:» always wins. The same cascade is available after import: entering the theme author, or the package editor on a `SharedEditors` Своя гра package, fills every question that still has none (a question under a theme/block that has its own editors is left alone)
